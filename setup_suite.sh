...
stag=cdcon_weak
ncols=32
a2cores_per_node=128


# Maximum jobs to set up (2**19 = 524288 cores, 4096/5276 ARCHER2 nodes)
min_sq=1
max_sq=10


#complist="bmc bmcs bmcd bmg"
#for cnc in $complist ; do

# Loop over core count exponent
for pnc in $(seq $min_sq $max_sq) ; do
  ncores=$((2**${pnc}))
...
#  fi
  ...
  # #PBS -l walltime=add_walltime
    sed -i -e 's/add_walltime/'"$internal:20:00"'/g' $pbs
